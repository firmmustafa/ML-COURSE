...
X = points.iloc[:, 1: ].values # Can take any number of numerical features 
...
X[:, (X.shape[1]-2)] = labelencoder_X.fit_transform(X[:, (X.shape[1]-2)])
onehotencoder = OneHotEncoder(categorical_features = [(X.shape[1]-2)])
...
print('Optimized thetas using gradient descent ->',g.size)
